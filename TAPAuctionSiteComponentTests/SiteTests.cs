...
namespace TAP21_22.AuctionSite.Testing {
    public class SiteTests : AuctionSiteTests {
        ...
        [Test]
        public void SameUsernameDifferentSite_DoesntThrowException() {
            TheHost.CreateSite("Sito 2", 0, 10, 2);
            var sito = TheHost.LoadSite("Sito 2");
            Site.CreateUser("ProvaUno", "ProvaUno");
            sito.CreateUser("ProvaUno", "ProvaUno");
        }

        [Test]
        public void DeleteSiteDisposeSession_Throws() {
            Site.CreateUser("User1", "User1");
            var session = Site.Login("User1", "User1");
            Site.Delete();
            Assert.That(() => session.Logout(), Throws.TypeOf<AuctionSiteInvalidOperationException>());
        }

        [Test]
        public void DeleteSiteDisposeUser_Throws() {
            Site.CreateUser("User1", "User1");
            var user = Site.ToyGetUsers().First();
            Site.Delete();
            Assert.That(() => user.WonAuctions(), Throws.TypeOf<AuctionSiteInvalidOperationException>());
        }

        [Test]
        public void DeleteSiteDisposeAuctions_Throws() {
            Site.CreateUser("User1", "User1");
            var session = Site.Login("User1", "User1");
            var auction = session.CreateAuction("Asta", Site.Now().AddSeconds(20), 50);
            TheClock.AddSeconds(1000);
            Site.Delete();
            Assert.That(() => auction.CurrentPrice(), Throws.TypeOf<AuctionSiteInvalidOperationException>());
        }

        [Test]
        public void ToyGetUsersWithoutUsers() {
            Assert.That(Site.ToyGetUsers().Count(), Is.EqualTo(0));
        }
    }
}