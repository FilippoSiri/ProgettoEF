...
namespace TAP21_22.AuctionSite.Testing {
    public class UserTests : AuctionSiteTests {
        ...
        [Test]
        public void DeleteUserSetNullAuctions() {
            var session = Site.Login(UserName, Pw);
            var auction = session.CreateAuction("Auction", Site.Now().AddSeconds(20), 50);
            Site.CreateUser("User2", "User2");
            var session2 = Site.Login("User2", "User2");
            auction.Bid(session2, 100);
            TheClock.AddSeconds(1000);
            session2.User.Delete();
            Assert.That(auction.CurrentWinner(), Is.EqualTo(null));
        }
    }
}